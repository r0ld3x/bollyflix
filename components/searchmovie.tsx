...
export default function SearchMovie(mid: { mid: string }) {
  const id = mid.mid;
  return (
...
async function Albums(query: any) {
  try {
    const response = await fetch(
      `https://api.themoviedb.org/3/search/movie?query=${query.query}&api_key=${process.env.API_KEY}`,
      { cache: "no-cache" }
    );
